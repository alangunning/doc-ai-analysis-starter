...
def _repl_edit_url_list(args: list[str]) -> None:
    ...
    if doc_type is None:
        ...
        try:
            ...
            logger.exception("Prompt for document type failed")
            click.echo("Prompt failed; please provide the document type.")
            doc_type = None
    if not doc_type:
        click.echo("Document type required")
        return
...
def interactive_shell(app: typer.Typer, init: Path | None = None) -> None:
    ...
    if init is not None:
        try:
            ...
            click.echo(f"Failed to run batch file {init}: {exc}")
